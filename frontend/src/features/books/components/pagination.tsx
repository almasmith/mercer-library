...
export function Pagination({ totalItems }: { totalItems: number }) {
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div className="text-sm text-slate-600">{from}–{to} of {totalItems}</div>
      <div className="flex items-center gap-2">
        <label className="text-sm">Page size</label>
        <select className="rounded border px-2 py-1" value={pageSize}
          onChange={(e) => { set("pageSize", e.target.value); set("page", "1"); }}>
          {sizeOptions.map(s => <option key={s} value={s}>{s}</option>)}
        </select>
        <button className="rounded border px-2 py-1 disabled:opacity-50" disabled={disablePrev}
        ...
        <button className="rounded border px-2 py-1 disabled:opacity-50" disabled={disableNext}
